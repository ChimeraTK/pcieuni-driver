...
public:
    PcieuniTestSuite(std::string const & deviceFileName) :
            test_suite("PCIeuni test suite") {
        // create an instance of the test class
        boost::shared_ptr<ReaderWriter> readerWriter(new T(deviceFileName));

        boost::shared_ptr<PcieuniTest> pcieuniTest(
                new PcieuniTest(readerWriter));
        ...
        add(BOOST_CLASS_TEST_CASE(&PcieuniTest::testUserFunctions,
                pcieuniTest));
    }
};
...
init_unit_test_suite(int /*argc*/, char* /*argv*/[]) {
    ...
    framework::master_test_suite().add(
            new PcieuniTestSuite<NormalReaderWriter>(
                    std::string("/dev/") + PCIEUNI_NAME + PCIEUNI_SLOT));

    return NULL;
}

PcieuniTest::PcieuniTest(boost::shared_ptr<ReaderWriter> const & readerWriter) :
...
void PcieuniTest::testIoctl() {
    ...
    BOOST_CHECK_NO_THROW(
            _readerWriter->ioctlExec(PCIEUNI_DRIVER_VERSION, &ioData));
    ...
    BOOST_CHECK_NO_THROW(
            _readerWriter->ioctlExec(PCIEUNI_FIRMWARE_VERSION, &ioData));
    BOOST_CHECK(ioData.data > 0);
    printf("FIRMWARE VERSION IS %X\n", ioData.data);

    BOOST_CHECK_NO_THROW(
            _readerWriter->ioctlExec(PCIEUNI_PHYSICAL_SLOT, &ioData));
    BOOST_CHECK(ioData.data > 0);
    ...
    BOOST_CHECK_NO_THROW(
            _readerWriter->ioctlExec(PCIEUNI_GET_DMA_TIME, &timeData));
...
void PcieuniTest::testReadWrite() {
    ...
    BOOST_CHECK_NO_THROW(
            _readerWriter->readSingle(WORD_TIMING_FREQ + 0x00, 1, 4));
    BOOST_CHECK_NO_THROW(
            _readerWriter->writeSingle(WORD_RESET_N + 0x00, 0, 4, 1));
    BOOST_CHECK_NO_THROW(
    ...
    BOOST_CHECK_THROW(_readerWriter->readSingle(WORD_RESET_N, 0, 3),
            DeviceIOException);
    BOOST_CHECK_THROW(_readerWriter->writeSingle(WORD_RESET_N, 0, 3, 1),
            DeviceIOException);

    // Try to read/write to offset that is not a multiple of 4
    BOOST_CHECK_THROW(_readerWriter->readSingle(WORD_RESET_N + 1, 0, 4),
            DeviceIOException);
    BOOST_CHECK_THROW(_readerWriter->writeSingle(WORD_RESET_N + 1, 0, 4, 1),
            DeviceIOException);

    // Try to read/write over bar 5
    BOOST_CHECK_THROW(_readerWriter->readSingle((6LL) << 60, 0, 4),
            DeviceIOException);
    BOOST_CHECK_THROW(_readerWriter->writeSingle((6LL) << 60, 0, 4, 1),
            DeviceIOException);
}

void PcieuniTest::testUserFunctions() {
    BOOST_CHECK_NO_THROW(_readerWriter->procFileTest(std::string("/proc/")
            + PCIEUNI_NAME + PCIEUNI_SLOT));
}
