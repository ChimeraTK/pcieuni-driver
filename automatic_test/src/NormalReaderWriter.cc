...
void NormalReaderWriter::readArea(uint64_t offset, uint32_t bar, uint32_t count,
    ...
    if (pread(_fileDescriptor, readBuffer, nWords * count,
            virtualOffset) != nWords * count) {
        throw DeviceIOException("Error reading from device");
    }
}

void NormalReaderWriter::writeSingle(uint64_t offset, uint32_t bar, uint32_t count,
        int32_t value) {
    writeArea(offset, bar, count, 1, &value);
}

void NormalReaderWriter::writeArea(uint64_t offset, uint32_t bar, uint32_t count,
    ...
    off_t virtualOffset = PCIEUNI_BAR_OFFSETS[bar] + offset;
    ssize_t writeStatus = pwrite(_fileDescriptor, writeBuffer,
            nWords * count, virtualOffset);
